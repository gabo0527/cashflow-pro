...
// Mano CG overhead detection
const OVERHEAD_CLIENT_NAMES = ['mano cg', 'mano cg llc', 'manogc', 'mano']

function isOverheadClient(clientName: string | undefined): boolean {
  if (!clientName) return false
  return OVERHEAD_CLIENT_NAMES.some(n => clientName.toLowerCase().includes(n))
}
...
// Billable / Overhead badge
function BillableBadge({ isBillable, isOverhead }: { isBillable: boolean; isOverhead: boolean }) {
  if (isOverhead && !isBillable) {
    return (
...
const expenseGridCols = 'grid-cols-[1fr_120px_120px_80px_75px_85px_85px_170px]'
...
function ClientAccordion({ clientName, clientId, invoices, isOverhead, isExpanded, onToggle, children }: {
  clientName: string; clientId: string; invoices: ContractorInvoice[]; isOverhead: boolean; isExpanded: boolean; onToggle: () => void; children: React.ReactNode
}) {
      ...
      <button onClick={onToggle} className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${isOverhead ? 'bg-gray-100 text-gray-500' : 'bg-emerald-50 text-emerald-700'}`}>
              {clientName.charAt(0).toUpperCase()}
            </div>
            <div className="text-left">
              <div className="flex items-center gap-2">
                <span className="text-gray-900 text-sm font-semibold">{clientName}</span>
                {isOverhead && <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 uppercase">Overhead</span>}
              </div>
              <span className="text-gray-400 text-xs">{invoices.length} invoice{invoices.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
          {/* Status pills */}
          <div className="flex items-center gap-1.5 ml-4">
            ...
            {scheduledCount > 0 && (
              <span className="text-purple-700 text-[10px] font-semibold bg-purple-50 border border-purple-200 px-2 py-0.5 rounded-full">
                {scheduledCount} scheduled
              </span>
            )}
            {paidCount > 0 && (
              <span className="text-emerald-600 text-[10px] font-semibold bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
                {paidCount} paid
              </span>
            )}
          </div>
        </div>
        <span className="text-gray-900 text-base font-semibold vN">{formatCurrency(total)}</span>
      </button>
      {isExpanded && children}
    </div>
  )
...
// ============ EXPENSE CLIENT ACCORDION ============
function ExpenseClientAccordion({ clientName, isOverhead, expenses, isExpanded, onToggle, children }: {
  clientName: string; isOverhead: boolean; expenses: ContractorExpense[]; isExpanded: boolean; onToggle: () => void; children: React.ReactNode
}) {
  const total = expenses.reduce((s, e) => s + e.amount, 0)
  const pendingCount = expenses.filter(e => e.status === 'pending').length
  const billableCount = expenses.filter(e => e.is_billable).length
  const billableAmt = expenses.filter(e => e.is_billable).reduce((s, e) => s + e.amount, 0)
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              ...
              <span className="text-gray-400 text-xs">{expenses.length} expense{expenses.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 ml-4">
            {pendingCount > 0 && (
            ...
            {billableCount > 0 && (
              <span className="text-emerald-600 text-[10px] font-semibold bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
                {billableCount} billable · {formatCurrency(billableAmt)}
              </span>
            )}
          </div>
        </div>
...
export default function ContractorManagement() {
  ...
  // Client accordion state
  const [collapsedClients, setCollapsedClients] = useState<Set<string>>(new Set())
  const toggleClient = (key: string) => setCollapsedClients(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n })

  // Expense client accordion state
  const [collapsedExpClients, setCollapsedExpClients] = useState<Set<string>>(new Set())
  const toggleExpClient = (key: string) => setCollapsedExpClients(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n })
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Default billable logic: Mano CG = non-billable (overhead), others = billable
      const rawExpenses = expRes.data || []
      const clientList = clientRes.data || []
      setExpenses(rawExpenses.map((e: any) => ({
        ...e,
        is_billable: e.is_billable !== null && e.is_billable !== undefined
          ? e.is_billable
          : (e.client_id ? !isOverheadClient(clientList.find((c: any) => c.id === e.client_id)?.name) : false)
      })))
  ...
  // ============ INVOICES GROUPED BY CLIENT (always) ============
  const invoicesByClient = useMemo(() => {
    const groups: Record<string, { clientId: string; clientName: string; isOverhead: boolean; invoices: ContractorInvoice[] }> = {}
    
    filteredInvoices.forEach(inv => {
      const lines = inv.contractor_invoice_lines || []
      if (lines.length === 0) {
        const key = '_unassigned'
        if (!groups[key]) groups[key] = { clientId: '', clientName: 'Unassigned', isOverhead: false, invoices: [] }
        groups[key].invoices.push(inv)
      } else {
        const clientTotals: Record<string, number> = {}
        lines.forEach((l: any) => { clientTotals[l.client_id] = (clientTotals[l.client_id] || 0) + l.amount })
        const primaryClientId = Object.entries(clientTotals).sort((a, b) => b[1] - a[1])[0]?.[0] || '_unassigned'
        const key = primaryClientId
        const name = clientMap[key] || 'Unknown'
        if (!groups[key]) groups[key] = { clientId: key, clientName: name, isOverhead: isOverheadClient(name), invoices: [] }
        groups[key].invoices.push(inv)
      }
    })
    ...
    return Object.values(groups).sort((a, b) => {
      ...
      if (!a.isOverhead && b.isOverhead) return -1
      return b.invoices.reduce((s, i) => s + i.total_amount, 0) - a.invoices.reduce((s, i) => s + i.total_amount, 0)
    })
  ...
  const expensesByClient = useMemo(() => {
    const groups: Record<string, { clientId: string; clientName: string; isOverhead: boolean; expenses: ContractorExpense[] }> = {}
    
    filteredExpenses.forEach(exp => {
      const key = exp.client_id || '_unassigned'
      const name = exp.client_id ? (clientMap[exp.client_id] || 'Unknown') : 'Unassigned'
      if (!groups[key]) groups[key] = { clientId: exp.client_id || '', clientName: name, isOverhead: isOverheadClient(name), expenses: [] }
    ...
    return Object.values(groups).sort((a, b) => {
      if (a.isOverhead && !b.isOverhead) return 1
      if (!a.isOverhead && b.isOverhead) return -1
      return b.expenses.reduce((s, e) => s + e.amount, 0) - a.expenses.reduce((s, e) => s + e.amount, 0)
    })
  }, [filteredExpenses, clientMap])
  ...
  const updateExpenseStatus = async (id: string, status: string, isBillable?: boolean) => {
    setProcessing(id)
    const updates: any = { status, reviewed_at: new Date().toISOString() }
  ...
  const toggleExpenseBillable = async (id: string, currentBillable: boolean) => {
    const newVal = !currentBillable
    const { error } = await supabase.from('contractor_expenses').update({ is_billable: newVal }).eq('id', id)
    if (!error) {
      setExpenses(prev => prev.map(exp => exp.id === id ? { ...exp, is_billable: newVal } : exp))
    }
  }
    ...
    <div className="space-y-5">
      ...
      {activeTab === 'invoices' && (
        <div className="space-y-4">
          ...
          {/* ===== INVOICE LIST — COLLAPSIBLE BY CLIENT ===== */}
          {filteredInvoices.length > 0 ? (
            <div className="space-y-3">
              {invoicesByClient.map(group => (
                <ClientAccordion
                  key={group.clientId || '_unassigned'}
                  clientName={group.clientName}
                  clientId={group.clientId}
                  invoices={group.invoices}
                  isOverhead={group.isOverhead}
                  isExpanded={!collapsedClients.has(group.clientId || '_unassigned')}
                  onToggle={() => toggleClient(group.clientId || '_unassigned')}
                >
      ...
      {activeTab === 'expenses' && (
        <div className="space-y-4">
          ...
          {/* ===== EXPENSE LIST — COLLAPSIBLE BY CLIENT ===== */}
          {filteredExpenses.length > 0 ? (
            <div className="space-y-3">
              {expensesByClient.map(group => (
                <ExpenseClientAccordion
                  key={group.clientId || '_unassigned'}
                  clientName={group.clientName}
                  isOverhead={group.isOverhead}
                  expenses={group.expenses}
                  isExpanded={!collapsedExpClients.has(group.clientId || '_unassigned')}
                  onToggle={() => toggleExpClient(group.clientId || '_unassigned')}
                >
                  <div className={`grid ${expenseGridCols} gap-2 px-5 py-2.5 border-b border-gray-200 bg-gray-50`}>
                    ...
                    <span className={`vLbl text-[10px] ${THEME.textDim} font-bold uppercase`}>Contractor</span>
                    <span className={`vLbl text-[10px] ${THEME.textDim} font-bold uppercase`}>Client</span>
                    ...
                    <span className={`vLbl text-[10px] ${THEME.textDim} font-bold uppercase text-right`}>Amount</span>
                    <span className={`vLbl text-[10px] ${THEME.textDim} font-bold uppercase text-center`}>Billable</span>
                    <span className={`vLbl text-[10px] ${THEME.textDim} font-bold uppercase`}>Actions</span>
                  </div>
                  ...
                  {group.expenses.map(exp => {
                    ...
                    const expClientName = exp.client_id ? clientMap[exp.client_id] || '—' : '—'
                    const overhead = isOverheadClient(expClientName)
                    const billable = exp.is_billable ?? false
                    ...
                    return (
                      <div key={exp.id} className={`grid ${expenseGridCols} gap-2 px-5 py-3 items-center border-b border-gray-100 last:border-0 ${THEME.cardHover} transition-colors ${getRowBorder(exp.status)}`}>
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            <div className={`w-5 h-5 rounded flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${overhead ? 'bg-gray-100 text-gray-500' : 'bg-emerald-50 text-emerald-700'}`}>
                              {expClientName !== '—' ? expClientName.charAt(0).toUpperCase() : '?'}
                            </div>
                            <span className={`text-xs truncate ${overhead ? 'text-gray-500' : 'text-gray-700 font-medium'}`}>{expClientName}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex justify-center">
                          <button
                            onClick={() => toggleExpenseBillable(exp.id, billable)}
                            className="group"
                            title={billable ? 'Click to mark non-billable' : 'Click to mark billable'}
                          >
                            <BillableBadge isBillable={billable} isOverhead={overhead && !billable} />
                          </button>
                        </div>
                        <div className="flex items-center gap-1.5">
                          {exp.receipt_url && (() => {
                          ...
                          {exp.status === 'pending' && (
                            <>
                              <button onClick={() => updateExpenseStatus(exp.id, 'approved', billable)} disabled={isProcessing}
                                className="px-2.5 py-1.5 rounded-lg bg-emerald-500 text-white text-xs font-semibold hover:bg-emerald-600 disabled:opacity-50 vBtn">
                                {isProcessing ? <Loader2 size={12} className="animate-spin" /> : 'Approve'}
                              </button>
